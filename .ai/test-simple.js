...
async function runTests() {
  ...
  await apiCall(
    ...
    '/api/sets',
    null,
    'List Sets (Expected: 401 Unauthorized without token)'
  );

  // Test 3: Invalid UUID format
  await apiCall(
    'GET',
    '/api/sets/invalid-uuid',
    null,
    'Get Set with Invalid UUID (Expected: 400 Bad Request)'
  );
  ...
  await apiCall(
    ...
    '/api/sets',
    {},
    'Create Set with Missing Fields (Expected: 400 Validation Error)'
  );
  ...
  await apiCall(
    ...
    '/api/srs/due',
    null,
    'Get Due Cards (Expected: 401 Unauthorized without token)'
  );
