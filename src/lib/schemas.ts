...
export const UpdateCardSchema = z
  .object({
    front: z.string().min(1, "Front text is required").max(200, "Front text must not exceed 200 characters").optional(),
    back: z.string().min(1, "Back text is required").max(500, "Back text must not exceed 500 characters").optional(),
  })
...
export const BatchCreateCardItemSchema = z.object({
  front: z.string().min(1, "Front text is required").max(200, "Front text must not exceed 200 characters"),
  back: z.string().min(1, "Back text is required").max(500, "Back text must not exceed 500 characters"),
  source_text_excerpt: z.string().max(500, "Source text excerpt must not exceed 500 characters").optional(),
  ...
  original_front: z.string().max(200, "Original front must not exceed 200 characters").nullable().optional(),
  original_back: z.string().max(500, "Original back must not exceed 500 characters").nullable().optional(),
});
...
export const BatchCreateCardsSchema = z.object({
  ...
  cards: z
    .array(BatchCreateCardItemSchema)
...
export const MigrationRequestSchema = z
  .object({
    ...
    newSetName: z.string().min(1, "Nazwa zestawu jest wymagana").max(100, "Nazwa zestawu zbyt długa").optional(),
  })
  .refine(
