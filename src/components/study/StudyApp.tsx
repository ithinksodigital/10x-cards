...
export function StudyApp({ setId }: StudyAppProps) {
  ...
  const handleStartSession = useCallback(async () => {
    ...
    try {
      ...
      // Check if session has any cards
      if (!response.cards || response.cards.length === 0) {
        // More specific error message based on response
        let errorMsg = "Brak kart dostępnych do nauki w tym momencie.";
        if (response.new_cards === 0 && response.review_cards === 0) {
          errorMsg =
            "Brak kart do powtórki. Wszystkie karty z tego zestawu są już przerobione na dziś lub nie są jeszcze gotowe do powtórki (sprawdź ponownie później).";
        } else if (response.new_cards === 0) {
          errorMsg = "Brak nowych kart. Możliwe, że osiągnięto dzienny limit nowych kart (20/dzień).";
        } else if (response.review_cards === 0) {
          errorMsg =
            "Brak kart do powtórki. Wszystkie karty są już przerobione na dziś lub nie są jeszcze gotowe do powtórki.";
        }
        setErrorMessage(errorMsg);
        setState("error");
        return;
  ...
  if (state === "summary" && summary) {
    return <SessionSummary summary={summary} onStudyMore={handleStudyMore} onBackToSets={handleBackToSets} />;
  }
  ...
  return (
    <div className="min-h-screen bg-background py-8 px-4">
      <div className="container mx-auto max-w-4xl">
        <div className="mb-6">
          <Button variant="ghost" onClick={handleBackToSets} className="mb-4">
            <ArrowLeft className="h-4 w-4 mr-2" />
            Powrót do zestawów
          </Button>
            ...
            <CardContent>
              {setInfo.cards_count === 0 ? (
                ...
                <div className="space-y-4">
                  ...
                  <Button onClick={handleStartSession} disabled={isLoading} size="lg" className="w-full sm:w-auto">
                    {isLoading ? (
          ...
          <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
            <Card>
              ...
              <CardContent>
                ...
                <Button onClick={handleBackToSets} variant="outline" className="w-full">
                  Przejdź do zestawów
                </Button>
              </CardContent>
            </Card>
